...
@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            ...
            @endif
            <div class="card-body">
                <form method="POST" action="{{ route('update.role', $roles->id) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label class="text-strong" for="name">Tên vai trò</label>
                        <input class="form-control" type="text" value="{{ $roles->name }}" name="name"
                            id="name">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" value="{{ $roles->role_description }}"
                                ...
                                @foreach ($permissions as $key => $per)
                                    <div class="col-md-3">
                                        <input type="checkbox" class="checkbox_child" value="{{ $per->id }}"
                                            ...
                                            {{ $roles->hasPermissionTo($per->name) ? 'checked' : '' }}>
